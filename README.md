...
| [`dashboard_screenshot_guide.md`](examples/outputs/dashboard_screenshot_guide.md) | Exact public dashboard screenshot capture commands, source hashes, screenshot receipt path, and no-live-data/no-broker/no-advice boundaries |
...
Here "public" means generated demo/review artifacts intended for sharing after
review. Do not publish private holdings, account exports, or screenshots that
contain account identifiers.

Write the dashboard screenshot guide after the dashboard and public review
packet have been refreshed:

```bash
portfolio-risk-compass screenshot-guide \
  --manifest examples/outputs/index.json \
  --markdown examples/outputs/dashboard_screenshot_guide.md \
  --json examples/outputs/dashboard_screenshot_guide.json
```

The screenshot guide records the exact Chromium capture command, viewport,
source hashes, expected screenshot path, and the same no-live-data, no-broker,
and no-advice boundaries. The screenshot image itself is generated by reviewers
when visual evidence is needed and is not required for the default release
artifact set.
